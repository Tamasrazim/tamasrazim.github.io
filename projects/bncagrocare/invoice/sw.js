...
self.addEventListener('fetch',e=>{if(e.request.method!=='GET')return;const u=new URL(e.request.url);const same=u.origin===self.location.origin;const pdfLib=[PDF_LIB_URL,PDFJS_URL,PDFJS_WORKER_URL].includes(u.href);if(!same&&!pdfLib)return;e.respondWith(caches.match(e.request).then(c=>c||fetch(e.request).then(r=>{if(r.ok){const copy=r.clone();caches.open(CACHE).then(x=>x.put(e.request,copy)).catch(()=>{})}return r}).catch(()=>caches.match('./index.html').then(r=>r||caches.match(FALLBACK_URL)))))});
...
**Customer → products → BNCFINAL.xlsx → live workbook preview → download XLSX**

The base sheet is **4 rows per side / 8 product slots**. Adding a row follows `reference/addProductRow.js`: insert before the ST subtotal region, copy the row structure, extend the ST formulas, and renumber left/right SL values.

The source workbook is loaded fresh for every export. The repository copy of `reference/BNCFINAL.xlsx` is never modified. Invoice Studio exports a new `.xlsx` file locally.

No PDF is generated by Invoice Studio.

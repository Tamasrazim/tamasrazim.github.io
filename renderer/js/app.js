...
function renderAt(t){
  if(!userFn)return false;t=Math.max(0,Math.min(DURATION,Number(t)||0));frame=Math.floor(t*FPS);window.__rendererTransparent=transparent.checked;
  try{userFn(effectiveTime(t),effectiveFrame(t),FPS);timeline.value=t;timeLabel.textContent=t.toFixed(3)+'s';frameLabel.textContent=frame+' / '+Math.max(1,Math.round(DURATION*FPS));canvasInfo.textContent=timeLabel.textContent+' / frame '+frame;progress.style.transform='scaleX('+(DURATION?t/DURATION:0)+')';return true}
  ...
  try{
    ...
    if(videoEngine.value==='webcodecs'){
      ...
      result=await window.TamasrazimVideoEngine.render({
        ...
        renderFrame:function(t,f,fr){window.__rendererTransparent=transparent.checked;userFn(t,f,fr)},
...
$$('.tab').forEach(function(tab){tab.addEventListener('click',function(){$$('.tab').forEach(function(x){x.classList.remove('active')});tab.classList.add('active');['codePanel','settingsPanel','stockPanel'].forEach(function(id){$('#'+id).hidden=id!==tab.dataset.tab})})});
...
try{var saved=localStorage.getItem('tamasrazim-renderer-code');if(saved)codeEl.value=saved}catch(e){}
window.addEventListener('beforeunload',function(){try{localStorage.setItem('tamasrazim-renderer-code',codeEl.value)}catch(e){}});
